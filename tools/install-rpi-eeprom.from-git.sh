...
detect_soc() {
  ...
  if [ -z "$soc" ] && command -v vcgencmd >/dev/null 2>&1; then
    ...
    if vcgencmd bootloader_config 2>/dev/null | grep -q '.*'; then
      # Pi 4 era boards typically = 2711 if compatible not readable
      soc="2711"
    fi
  fi
  if [ -z "$soc" ]; then
...
if [ ! -d "$TMP/$SOC_DIR" ]; then
  # Fallback for older trees: try generic 'firmware' if SoC dir missing
